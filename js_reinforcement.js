...
project.steps.forEach(function(proj) {
  proj.member = project.committee[Math.floor(Math.random() * project.committee.length)];
})

for (var k in project.steps) {
    // use hasOwnProperty to filter out keys from the Object.prototype
    if (project.steps.hasOwnProperty(k)) {
        console.log(' is: ' + k + ', value is: ' + project.steps[k]);
    }
}
